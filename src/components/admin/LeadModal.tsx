      ...
      <DialogContent className="max-h-[85vh] max-w-4xl overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-3">
          <div>
            ...
            <p className="mt-0.5 text-sm">{lead.email}</p>
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
              Phone
          ...
          <div>
            ...
            <span
              className={`mt-1 inline-flex items-center rounded-md px-2.5 py-1 text-xs font-medium ${STATUS_COLORS[lead.status]}`}
            >
        ...
        <div className="space-y-4">
          ...
          {/* Existing Notes (chronological) */}
          {lead.notes.length > 0 && (
            <div className="max-h-52 space-y-2.5 overflow-y-auto rounded-lg border border-border/50 bg-muted/20 p-3">
              {lead.notes.map((note) => (
                <div
                  key={note.id}
                  className="group rounded-lg border border-border/40 bg-background p-3"
                >
